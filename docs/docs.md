...
options:
  ...
  --output-path OUTPUT_PATH
                        Relative path to where calibration file will be written. Defaults to 'processing'. Accepts any pathtype, but
                        will be relative by default. Extended paths (e.g. `processing/my/experiment`) can be given, directories will
                        be created if not already found
  --outname OUTNAME     Name of output calibration file. Defaults to 'SAXS_calibration'
  --no-save-plots       Don't save processing plots alongside the calibration file
  --no-show-plots       By default, the final calibration plot is shown interactively when the program is run. use this flag to turn
                        this behaviour off and not show any plots
  ```
...
- `--no-show-plots`
    * By default, the program will show the final plot it makes: a figure showing how the detector pattern has been integrated, the peaks fitted & indexed, and the q/index plot. This is generated as a single compound figure, see below. This plot will have to be interactively closed when it appears before the program ends. If you don't want this plot shown, and will inspect it manually afterwards instead, then use this flag.
