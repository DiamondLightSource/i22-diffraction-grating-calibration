import pytest
import numpy as np
from nexusformat.nexus import (NXfield, NXentry, NXroot,   
                               NXmonochromator, NXdetector, 
                               NXinstrument
                               )
...
def make_fake_file(tmp_path_factory):
    ...
    def add_spot(xx, yy, cx, cy, w, r, I):
        dist = np.sqrt((xx - cx)**2 + (yy - cy)**2)
        return I * (1 - np.tanh((dist - r) / w))
    ...
    def _make_fake_file(
        ...
        detector_mask=((225,275),(425, 475),(725,775),
                       (1025,1075),(1225,1275),(1425, 1475))

    ):
        # --- cache key ---
        key = (cx, cy, r, spot_rows, background_intensity, spot_intensity, detector_shape)
        if key in cache:
            return cache[key]
        ...
        # calibration spots
        for i,j in enumerate(spot_rows,1):
