...
from nexusformat.nexus import (NXfield, NXdata, 
                               NXsample, NXbeam, NXdetector, 
                               NXdetector_module, NXtransformations, 
                               NXentry, NXinstrument, NXroot,
                               NXlink,
                               NeXusError)
import numpy as np
...
class CalibrantFileWriter:
    ...
    def sanitise_units(self): 
        ...
        length_converter = {"angstrom": {"m": 1e10,
                                         "nm": 10},
                            "nm": {"angstrom": 10}
                                         }

        desired_units = {'prop': {"angstrom": {"m": 1e10,
                                                     "nm": 10},
                                        "nm": {"angstrom"}}}
    ...
    def writer(self):
        ...
        detector_vector = np.array([beam_center.get('x'),
                                    beam_center.get('y'),
                                    detector_distance.get('value')])

        # this gets used a couple of times        
        nx_data = NXfield(detector_image, dtype= 'int32', 
        ...
        beam = NXbeam()
        beam.incident_wavelength = NXfield(wavelength.get('value'),
                                           dtype='float64', 
                                           units = wavelength.get('units'))
        nx_calibration_sample.beam = beam
        
        # detector
        nx_detector = NXdetector()
        nx_detector.beam_center_x = NXfield(beam_center.get('x'), 
                                            dtype = 'float64', 
                                            units = beam_center.get('units'))
        nx_detector.beam_center_y = NXfield(beam_center.get('y'), 
                                            dtype = 'float64', 
                                            units = beam_center.get('units'))
        nx_detector.data = nx_data
        nx_detector.depends_on = NXfield('./transformations/euler_c')
        nx_detector.x_pixel_size = NXfield(pixel_size.get('value'), 
                                           dtype = 'float64', 
                                           units = pixel_size.get('units'))
        nx_detector.y_pixel_size = NXfield(pixel_size.get('value'), 
                                           dtype = 'float64', 
                                           units = pixel_size.get('units'))
        nx_detector.distance = NXfield(detector_distance.get('value'),
                                       dtype = 'float64', 
        ...
        nx_detector_module.fast_pixel_direction = NXfield(pixel_size.get('value'),
                                                          units= pixel_size.get('units'),
                                                          dtype='float64', 
        ...
        nx_detector_module.slow_pixel_direction = NXfield(pixel_size.get('value'),
                                                          units= pixel_size.get('units'),
                                                          dtype='float64', 
                                                          depends_on = './module_offset', 
                                                          offset = [0.00000000,
        ...
        nx_detector_module.module_offset = NXfield(0, 
                                                   units = 'mm',
                                                   dtype='int32', 
        ...
        nx_transformations.euler_a = NXfield(0,
                                             dtype='float64', 
                                             units = 'deg',
        ...
        nx_transformations.euler_b = NXfield(0, 
                                             dtype='float64', 
                                             units = 'deg',
                                             depends_on = './euler_a', 
                                             offset = [0.00000000,
        ...
        nx_transformations.euler_c = NXfield(0, 
                                             dtype='float64', 
                                             units = 'deg',
                                             depends_on = './euler_b', 
                                             offset = [0.00000000,
        ...
        nx_transformations.origin_offset= NXfield(np.linalg.norm(detector_vector), 
                                                  dtype='float64', 
                                                  units = 'm',
                                                  depends_on = '.', 
                                                  offset = [0.00000000,
