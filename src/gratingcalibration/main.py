...
def main():
    ...
    parser.add_argument("--file", type=Path, default=None,
                        dest="input",
                        help="Path to input nexus file")
    parser.add_argument("--peak-prominance", type=float, default=0.1,
                        dest="peak_prominance",
                        help="Peak prominance to use in scipy.signal.find_peaks when finding grating fringes. Default=0.1")
    parser.add_argument("--output", type=Path, default="calibration",
                        dest="output",
                        help="Path to where calibration file will be written")
    parser.add_argument('--save-plots', action='store_false',
    ...
    a,b,c,d = (int(beamstop_center['x'] - 40), int(beamstop_center['x'] + 40),
               int(beamstop_center['y'] + 60), int(beamstop_center['y'] - 60))

    cropped = z_corr[d:c,a:b]
    cropped_center = {'x': beamstop_center.get('x') -  a, 
                      'y': beamstop_center.get('y') -  d
                      }
    
    cropped_mask = mask[d:c,a:b]
    
    fitter = BeamCenterOptimiser(image=cropped, 
                            mask=cropped_mask, 
    ...
    detector_calib = DetectorCalibration(image=z_corr,
    ...
    print(f"Detector located at {detector_calib.detector_distance:.5f} "
          "{detector_calib.detector_distance_units}. "
          "Writing calibration file.")

    data_out = {'image': data_in.raw_data,
                'wavelength': {'value': fitter.wavelength, 
                               'units': fitter.wavelength_units
                               }, 
                'pixel_size': {'value': 172e-6, 
                               'units': 'm'
                               },
                'beam_center': {'x': fitter.beam_center_global['x']*172e-6, 
                                'y': fitter.beam_center_global['y']*172e-6, 
                                'units': 'm'
                                },
                'detector_distance': {'value': detector_calib.detector_distance, 
    ...
    CalibrantFileWriter(datadict=data_out,
                        writepath= out / "SAXS_calibration.nxs").writer()
