...
import numpy as np
import pytest
from nexusformat.nexus import NeXusError, NXfield, nxload

from gratingcalibration.data_loader import DataLoader


def test_init_loads_everything(make_fake_file: Callable[..., Path]) -> None:
    """
    test we know how to load the fake file and that it has the expected data in
    """

    fake_data: Any = nxload(make_fake_file(), "r")
...
def test_detector_image_log_transform(make_fake_file: Callable[..., Path]) -> None:
...
def test_mask_creation(make_fake_file: Callable[..., Path]) -> None:
...
def test_energy_conversion_kev_to_ev(make_fake_file: Callable[..., Path]) -> None:
...
def test_invalid_datapath_raises(make_fake_file: Callable[..., Path]) -> None:
    with pytest.raises(NeXusError):
        DataLoader(make_fake_file(), datapath="invalid/path")


def test_invalid_energypath_raises(make_fake_file: Callable[..., Path]) -> None:
    with pytest.raises(NeXusError):
        DataLoader(make_fake_file(), energypath="invalid/path")


def test_log_does_not_apply_below_one(make_fake_file: Callable[..., Path]) -> None:
