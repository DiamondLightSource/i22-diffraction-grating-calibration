...
class FitBeamstop:
    ...
    def plot_circle_fit(self, image, fit_image, x0, y0, radius, extent):
        ...
        fig, (ax0, ax1, ax2) = plt.subplots(1, 3, figsize=(20, 7.5))
    ...
    def find_beamstop(self, plot=True):
        ...
        region = self.image[
            beamstop_y_min:beamstop_y_max, beamstop_x_min:beamstop_x_max
        ]

        # make it an actual peak for fitting purposes
        inverted = region.max() - region

        fit_result = self.fit_bright_circle(
            inverted,
            lo_x=beamstop_x_min,
            lo_y=beamstop_y_min,
        )
