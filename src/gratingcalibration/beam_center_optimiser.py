...
class BeamCenterOptimiser:
    def __init__(self, image, mask, beam_energy, beamstop_center, 
                 optimise_direction='x', offset = None):
        ...
        self.INTEGRATION_CONFIGS = {
            'y0':   dict(npt=50,  
                         azimuth_range=(90 - integration_range, 
                                        90 + integration_range),   
            ...
            'y1':   dict(npt=50,  
                         ...
                         radial_range=(0.005, 0.010)),
            'x0':   dict(npt=50,  
                         azimuth_range=(-integration_range, 
                                        integration_range),    
                         radial_range=(0.0023, 0.0065)),
            'x1a':  dict(npt=50,  
                         azimuth_range=(180 - integration_range, 
                                        180),
                         radial_range=(0.0023, 0.0065)),
            'x1b':  dict(npt=50,  
                         azimuth_range=(-180,
                                        -180 + integration_range),
                         radial_range=(0.0023, 0.0065)),
        }
    ...
    def _setup(self):
        ...
        if self.optimise_direction == 'x':
            ...
            self.target_configs = {key: value for key, value in 
                                   self.INTEGRATION_CONFIGS.items() if 'x' in key}
        elif self.optimise_direction == 'y':
            # fix x
            self.ai.poni2 = self.beamstop_center.get('x') * self.PILATUS2M_PIXEL_SIZE
            self.target_configs = {key: value for key, value in 
                                   self.INTEGRATION_CONFIGS.items() if 'y' in key}
    ...
    def _finalise_residual(self, q):
        ...
        if self.optimise_direction == 'x':
            ...
            Ix1 = 0.5 * (self._results_store['Ix1a'] + 
                         self._results_store['Ix1b'])
    ...
    def _make_beam_residual(self):
        ...
        def residual(pars, **kws):
            ...
            for key, kw in self.target_configs.items():
                q,I = self.ai.integrate1d(self.image, 
                                          mask=self.mask, 
                                          unit="r_m", 
                                          **kw
                                          )
                self._results_store[f'I{key}'] = I
    ...
    def fit_beam_centre(self):
        ...
        self.beam_center[self.optimise_direction] = result.params['beam_center_pos'].value
        if self.offset is not None:
            # need the -.5 correction for the rounding on the extent 
            # TODO: DOUBLE CHECK THIS
            self.beam_center_global[self.optimise_direction] = result.params['beam_center_pos'].value + self.offset.get(self.optimise_direction) - .5
    ...
    def profile_plotter(self):
        ...
        for ax_pair, profile_direction in zip(axarr, self.profiles.keys()):
            ...
            if data is not None:
                ...
                profiles = []
                for key, colour in zip(profile_keys, ['#4C9C88', '#9C5F4C']):
                    main.plot(data.get('q'),
                            data.get(key),
                            label=key,
                            c=colour,
                            lw=2)
                ...
                residual.plot(data.get('q'),
                      profile_diff, 
                      c='#332A31')
                residual.set_xlabel(f'Detector distance (m)\n(sos = {sum(profile_diff**2):.2f})',
                                    fontsize=15)
    ...
    def center_plotter(self, extent):
        ...
        ax.axvline(self.beamstop_center['x'] + extent[0],
                   ls=':',lw=1,
                   c='#ffffc1'
                   )
        ax.axhline(self.beamstop_center['y'] + extent[3],
                   ls=':',lw=1,
                   c='#ffffc1',
        ...
        ax.axhline(
                   self.beam_center.get('y') + extent[3],
                   c='#ff028d',
                   ls=':',lw=1
                   )
